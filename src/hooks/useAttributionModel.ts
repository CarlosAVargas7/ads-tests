...
export const useAttributionModel = (): {
  ...
  const calculateTimeDecayWeight = (
  ...
  const calculateAttributionModels = (
    data: AttributionData,
  ): AttributionData["attribution_models"] => {
    ...
    const linearModel = {
      sources: touch_history.map((touch) => ({
        source: touch.source,
        medium: touch.medium,
    ...
    const timeDecayModel = {
      sources: touch_history.map((touch) => {
        const weight = calculateTimeDecayWeight(
          touch.timestamp,
  ...
  const trackTouch = useCallback((touchData: any) => {
    ...
    if (!currentData) {
      ...
      // Returning user
      const isSameSource =
        currentData.last_touch.source === touch.source &&
      ...
      attributionData = {
        ...
        days_since_first_touch: Math.floor(
          (now - currentData.first_touch.timestamp) / (1000 * 60 * 60 * 24),
        ),
        hours_since_last_touch: Math.floor(
          (now - currentData.last_touch.timestamp) / (1000 * 60 * 60),
        ),
    ...
    attributionData.attribution_models =
      calculateAttributionModels(attributionData);
    ...
    window.dataLayer.push({
      ...
      attribution_models: {
        first_touch_source:
          attributionData.attribution_models.first_touch.source,
  ...
  const getAttributionData = (): AttributionData | null => {
    ...
    if (data) {
      ...
      const now = Date.now();
      data.hours_since_last_touch = Math.floor(
        (now - data.last_touch.timestamp) / (1000 * 60 * 60),
      );
      data.days_since_first_touch = Math.floor(
        (now - data.first_touch.timestamp) / (1000 * 60 * 60 * 24),
      );
    }
    return data;
  };
  ...
  const getAttributionInsights = () => {
    ...
    const insights = {
      user_journey: {
        ...
        is_multi_touch: data.is_multi_touch,
        complexity:
          data.total_touches === 1
            ? "simple"
            : data.total_touches <= 3
              ? "moderate"
              : "complex",
      },
      attribution_summary: {
      ...
      models_comparison: {
        ...
        time_decay_recent_focus:
          data.attribution_models.time_decay.sources[0]?.credit || 0,
      },
      recommendations: {
        ...
        optimization_focus: data.is_multi_touch
          ? "middle_funnel"
          : "top_funnel",
      },
    };
